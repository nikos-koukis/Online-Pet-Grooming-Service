...
export default function AdminDashboardLayout() {
    ...
    return (
        <div className={`bg-blue-600 h-screen p-5 pt-8 ${open ? "w-72" : "w-20"} duration-300 relative`}>
            <BsArrowLeftShort className={`bg-white text-2xl rounded-full absolute -right-3 top-9 border border-blue-600 cursor-pointer duration-300 
                ...
                {MenusItems.map((menu, index) => (
                    <>
                        <li key={index} className={`text-sm text-white flex items-center gap-x-4 cursor-pointer p-2
                         hover:bg-white hover:text-black rounded-md transition-all ${menu.spacing ? "mt-9" : "mt-2"} `}>
